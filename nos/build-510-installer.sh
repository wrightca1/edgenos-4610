#!/usr/bin/env bash
# build-510-installer.sh — reproducible EdgeNOS-4610 / Linux-5.10 onie-installer
...
#   - kernel  : clean 5.10 FIT = forward-ported 5.10.224 (cgroups) + RTC-disabled DTB
#               (-> /mnt/onl/boot/onl-loader-fit.itb)   [build-510-fit.sh]
#   - rootfs  : EdgeNOS-4610-510-ownbuild.swi = pure Buildroot (glibc+systemd, armhf
#               hard-float) + ko510 lockstep with the kernel + datapath autostart
#               (-> /mnt/onl/images)                    [build-ownbuild-swi.sh]
#   - boot-config : SWI=images:EdgeNOS-4610-510-ownbuild.swi  (explicit, NOT ::latest)
...
FIT="$T/output/kport510/arm-accton-as4610-54-r0-510.itb"      # clean 5.10 (cgroup) FIT
SWI="$T/output/kport510/EdgeNOS-4610-510-ownbuild.swi"        # pure Buildroot own-build SWI
STOCK_INST="$T/output/onie-installer-edgenos-419"            # source of installer.sh/sfx payload (generic)
OUT="$T/output/onie-installer-edgenos-510-ownbuild"
WORK="$T/output/inst510-build"             # under $EDGE so it's visible in the build container
...
# --- recompute the loader-initrd offset/size for the 5.10 FIT and patch installer.sh ---
...
echo "5.10 FIT initrd: offset=$OFF size=$SIZE"
