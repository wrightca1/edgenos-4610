...
docker run --rm -u root:0 -e SOURCE_DATE_EPOCH=1717480800 -v "$EDGE":"$EDGE" -w "$KP" "$IMG" bash -lc '
  ...
  gzip -n -9 -c '"$IMAGE"' > kernel-6.1-iproc.bin.gz
  mkimage -f '"$ITS"' arm-accton-as4610-54-r0-61.itb >/dev/null
'
( cd "$KP" && md5sum arm-accton-as4610-54-r0-61.itb )
echo "==> 6.1 FIT: $KP/arm-accton-as4610-54-r0-61.itb"
